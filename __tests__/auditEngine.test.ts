...
describe('Audit Engine Optimization Logic', () => {
  test('1. Cursor Business vs Pro Optimization', () => {
    const tools: ToolInput[] = [{ name: 'Cursor', plan: 'Business', monthlySpend: 160, seats: 4 }];
    const results = analyzeSpend(tools, 4, 'coding');
    expect(results[0].recommendedAction).toBe('Downgrade to Pro');
    expect(results[0].savings).toBe(80); // 160 - (4 * 20)
  });

  test('2. Claude Team Optimization', () => {
    const tools: ToolInput[] = [{ name: 'Claude', plan: 'Team', monthlySpend: 120, seats: 4 }];
    const results = analyzeSpend(tools, 4, 'mixed');
    expect(results[0].recommendedAction).toBe('Downgrade to Pro');
    expect(results[0].savings).toBe(40); // 120 - (4 * 20)
  });

  test('3. Copilot Enterprise Utilization', () => {
    const tools: ToolInput[] = [{ name: 'GitHub Copilot', plan: 'Enterprise', monthlySpend: 390, seats: 10 }];
    const results = analyzeSpend(tools, 10, 'coding');
    expect(results[0].recommendedAction).toBe('Downgrade to Business');
    expect(results[0].savings).toBe(200); // 390 - (10 * 19)
  });

  test('4. Credex Credit Opportunity for High Spend', () => {
    const tools: ToolInput[] = [{ name: 'OpenAI API', plan: 'Pay as you go', monthlySpend: 1000, seats: 10 }];
    const results = analyzeSpend(tools, 10, 'mixed');
    expect(results[0].recommendedAction).toBe('Purchase through Credex Credits');
    expect(results[0].savings).toBe(200); // 20% of 1000
  });

  test('5. Optimal Spend (No Savings)', () => {
    const tools: ToolInput[] = [{ name: 'Cursor', plan: 'Pro', monthlySpend: 20, seats: 1 }];
